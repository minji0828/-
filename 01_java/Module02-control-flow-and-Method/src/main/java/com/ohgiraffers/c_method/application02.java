...
*   클래스에 속하며 객체 생성 업시 호출할 수 있다.
*  예 : math.max(a,b)
*
* 2. 인스턴스 메서드(insttance method) :
*   객체에 속하며 객체를 생성한 후에 호출해야한다.
*   예 : string str = "hello"; str.length();
*
* 3. 생성자(constructor) :
*   클래스의 인스턴스를 생성할 때 호출되는 특별한 매서드이다.
*   생성자 반환 타입이 없고, 클래스 이름이 동일하다.
*
* 4. 오버로딩 (method overloading)
*   같은 이름의 메서드를 여러개 정의하되 매개변수의 타입이나 개수를 다르게하여 사용한다.
* 예 :
* public int add(int a, int b) {...}
* public double add(double a , double b) {...}
* 메서드 오버로딩을 통해 같은기능을 수행하는 다양한 메소드를 제공할 수 있다.
*
* */
