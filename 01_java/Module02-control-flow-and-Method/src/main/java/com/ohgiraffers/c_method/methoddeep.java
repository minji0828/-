...
* - 메서드가 호출되면 해당 메서드를 위한 공간인 스택프레임이 스택에 쌓인다.(push)
* - 이 스택 프레임에는 매개변수, 지역 변수, 리턴 주소 등이 저장된다.
* - 메서드 실행이 끝나면 ,스택 프레임은 스택에서 제거된다.(pop)
* - 장점 : 이런 구조 덕분에 메모의 관리가 용이하고 재귀 호출이 가능하며, 디버깅 시 호출 스택을 추적할 수 있다.
* */

/*
* 2.메서드와 성능의 관계 : JIT 컴파일러와 인라이닝
* - 메서드 호출에는 스택프레임을 쌓고 제거하는 미세한 비용(오버헤드)가 발생한다.
* - 하지만 현대 JVM은 매우 똑독해서 ,JIT 컴파일러를 통해 이 오버헤드를 최젇화한다.
* - JIT 컴파일러는 자주 실행되는 코드를 실시간으로 기계어로 번역하여 성능을 최적화한다.
* - 주요 최적화 기법 중 하나가 "인라이닝"이라는 것이다.
*
* */

/*3. 단일책임의 원칙
* - 메서드(또는 클래스)는 단 하나의 책임만 가져야한다.
