 ...
 *   HashMap은 체이닝 방식을 사용하여 연결 리스트로 저장한다.
 *   이로 인해 충돌된 요소들은 동일한 버킷 내에서 연결되어 관리된다.
 *
 * - 메모리 구조:
 *   HashMap은 힙 메모리에 버킷 배열을 사용하여 데이터를 저장한다.
 ...
 *
 * - 초기 용량 및 부하율:
 *   HashMap의 초기 용량을 설정할 수 있으며, 부하율(Load Factor)을 통해
 *   리사이징 시점을 조절하게되며, 부하율이 낮으면 충돌이 줄어들고
 *   성능이 향상될 수 있다. 예를 들어, 0.75f는 75%의 사용량에 도달했을 때
 *   리사이징이 발생한다.
 * = 리사이징이란 현재의 배열에 크기를 늘리고 기존 요소를 복사하는 과정을 의미한다.
 *   (짐은 그대로 가지고 가면서 큰집으로 이사하는 느낌)
 *
 * - 결론:
 *   HashMap은 빠른 데이터 접근과 유연한 키-값 쌍 관리를 제공하지만,
...
public class mapdeep {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(map.hashCode());
        /* 코드 설명 */
        // - 동일 키는 값 갱신, 해시 충돌 시 체이닝.
        // - 메모리: 힙 내 버킷 구조.
        System.out.println("HashMap: " + map);

        /* 최적화 */
        // 초기 map의 사이즈를 지정하여 리사이징이 생기지 않도록 한다.
