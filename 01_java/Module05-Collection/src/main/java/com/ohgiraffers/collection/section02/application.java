...
* = 데이터의 빠른 검색 및 접근이 간으하며 키를 통해 직접적으로 값을 조회할 수 있다.
*
* hash란?
* - hash는 데이터를 고정된 크기의 값으로 변환하는 과정을 의미한다. 일반적으로 해시 함수(hash funtion)을 통해 이루어진다.
* - 해시 함수는 입력값(예:키)를 받아 해시 코드(정수)를 생성하며, 이 해시코드는 hashMap의 내부 내열에서 인덱스로 사용된다.
