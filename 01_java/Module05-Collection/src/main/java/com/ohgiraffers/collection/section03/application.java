...
* - hash 테이블과 함게 이중 연결 리스트를 가지고 잇으며 이를 통해 다음번 연결값의 순서를 알 수 있게 된다.
...
*  - 정렬된 데이터를 필요로 할 때 유용하며, 범위 검색과 같은 작업을 쉽게 수행할 수 있다.
...
* 2. 순서 없음
* - hashSet은 요소의 순서르 보장하지않으며 요소의 순서는 예측할 수 없다.
* 3. 성능
* - hashSet은 평균적으로 O(1)의 시간복잡도로 데이터를 검색,삽입,삭제할 수 있다.
* 4. 메모리 사용
*  - hashSet은 내부적으로 해시테이블으 사용하기 때문에 메모리 용량이 크다.
*
* */
